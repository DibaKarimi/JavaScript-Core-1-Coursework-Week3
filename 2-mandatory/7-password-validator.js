...
function validatePasswords(passwords) {
  let result = [];
  result = passwords.map((password) => {
      return password.length > 4 && containsUppercaseLetter(password) && containsLowercaseLetter(password) && containsNumber(password) && containsSymbol(password);
  });
